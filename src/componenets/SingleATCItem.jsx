import { useQountAddedItem } from "../layouts/Main";
import { TiDelete } from "react-icons/ti";
export default function SingleATCItem({item, handleRemoveItemBtn}){
  const {product_title, product_id, product_image, category, price, color ,description , Specification, rating, availability} = item;

  const {addedItmeQuentity, setAddedItmeQuentity} = useQountAddedItem();
  return(
    <>
    <div className="bg-white p-5 rounded-xl flex flex-row items-center justify-between gap-5 max-sm:hidden">
      <div className="flex flex-col md:flex-row max-sm:items-center gap-4">
        <div className=" md:w-[150px] bg-[#9c00c31a] p-4 rounded-xl">
        ...
        <div className="flex flex-col items-center md:items-start gap-5">
          ...
          <p className="text-lg text-[#09080fcc]">Added Quentity ({addedItmeQuentity}) x $ {price}k = <span className="font-semibold">$ {price*addedItmeQuentity}k </span></p>
        </div>
      </div>
      <div className="">
